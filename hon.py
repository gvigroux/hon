...
class HonConnection():
    ...
    async def async_get_frontdoor_url(self, error_code = 0):
        data = "message=%7B%22actions%22%3A%5B%7B%22id%22%3A%2279%3Ba%22%2C%22descriptor%22%3A%22apex%3A%2F%2FLightningLoginCustomController%2FACTION%24login%22%2C%22callingDescriptor%22%3A%22markup%3A%2F%2Fc%3AloginForm%22%2C%22params%22%3A%7B%22username%22%3A%22"+urllib.parse.quote(self._email)+"%22%2C%22password%22%3A%22"+urllib.parse.quote(self._password)+"%22%2C%22startUrl%22%3A%22%22%7D%7D%5D%7D&aura.context=%7B%22mode%22%3A%22PROD%22%2C%22fwuid%22%3A%22"+urllib.parse.quote(self._framework)+"%22%2C%22app%22%3A%22siteforce%3AloginApp2%22%2C%22loaded%22%3A%7B%22APPLICATION%40markup%3A%2F%2Fsiteforce%3AloginApp2%22%3A%22YtNc5oyHTOvavSB9Q4rtag%22%7D%2C%22dn%22%3A%5B%5D%2C%22globals%22%3A%7B%7D%2C%22uad%22%3Afalse%7D&aura.pageURI=%2FSmartHome%2Fs%2Flogin%2F%3Flanguage%3Dfr&aura.token=null"
        async with self._session.post('https://he-accounts.force.com/SmartHome/s/sfsites/aura?r=3&other.LightningLoginCustom.login=1', headers = {'Content-Type':'application/x-www-form-urlencoded'}, data = data) as resp:
            if resp.status != 200:
            ...
            try:
                ...
                */{"event":{"descriptor":"markup://aura:clientOutOfSync","eventDef":{"descriptor":"markup://aura:clientOutOfSync","t":"APPLICATION","xs":"I"}},"exceptionMessage":"Framework has been updated. Expected: -SjNAdgW9yv96YgKI8MiFA Actual: ","exceptionEvent":true}/*ERROR*/
                ...
                if text.find("clientOutOfSync") > 0 and error_code != 2:
                    ...
                    _LOGGER.warning("Framework update from [" + self._framework + "] to [" + text[start:end] + "]")
    ...
    async def async_authorize(self):
        ...
        async with self._session.get("https://he-accounts.force.com/SmartHome/apex/ProgressiveLogin?retURL=%2FSmartHome%2Fapex%2FCustomCommunitiesLanding") as resp:
            wait_data = await resp.text()

        """ **** Get token *** """
        async with self._session.get("https://he-accounts.force.com/SmartHome/services/oauth2/authorize?response_type=token+id_token&client_id=3MVG9QDx8IX8nP5T2Ha8ofvlmjLZl5L_gvfbT9.HJvpHGKoAS_dcMN8LYpTSYeVFCraUnV.2Ag1Ki7m4znVO6&redirect_uri=hon%3A%2F%2Fmobilesdk%2Fdetect%2Foauth%2Fdone&display=touch&scope=api%20openid%20refresh_token%20web&nonce=82e9f4d1-140e-4872-9fad-15e25fbf2b7c") as resp:
            text = await resp.text()
            array = text.split('\'',2)
        ...
        async with self._session.post('https://api-iot.he.services/auth/v1/login', headers = post_headers, data = data) as resp:
            text = await resp.text()
            try:
                json_data = json.loads(text)
            except:
        ...
        credential_headers = {'cognito-token':self._cognitoToken,'id-token':self._id_token}
        async with self._session.get("https://api-iot.he.services/commands/v1/appliance", headers = credential_headers) as resp:
            text = await resp.text()
            try:
                json_data = json.loads(text)
            except:
    ...
    async def async_get_state(self,mac,typeName, loop = False):
        credential_headers = {'cognito-token':self._cognitoToken,'id-token':self._id_token}
        async with self._session.get("https://api-iot.he.services/commands/v1/context?macAddress="+mac+"&applianceType="+typeName+"&category=CYCLE", headers = credential_headers) as resp:
            text = await resp.text()
            ...
            if resp.status == 403:
                # Do only one retry
                if loop == True:
                    _LOGGER.error("Unable to get the state of the hOn device. HTTP code: " + str(resp.status) + " and text[" + text + "]")
                    return False
    ...
    async def async_set(self,mac,typeName,parameters):
        post_headers = {'Content-Type':'application/json','cognito-token':self._cognitoToken,'id-token':self._id_token}
        ...
        data["ancillaryParameters"] = json.loads("{\"programFamily\":\"[standard]\", \"remoteActionable\": \"1\", \"remoteVisible\": \"1\"}")
        data["applianceType"]       = typeName
        data["attributes"]          = json.loads("{\"prStr\":\"HOME_ASSISTANT\", \"channel\":\"googleHome\", \"origin\": \"conversationalVoice\"}")
        data["device"]              = json.loads('{"mobileId":"xxxxxxxxxxxxxxxxxxx", "mobileOs": "android", "osVersion": "31", "appVersion": "1.41.2", "deviceModel": "lito"}')
        data["parameters"]          = parameters
        data["timestamp"]           = timestamp
        data["transactionId"]       = mac + "_" + data["timestamp"]
        
        #_LOGGER.warning(data)
        async with self._session.post('https://api-iot.he.services/commands/v1/send', headers = post_headers, json = data) as resp:
            ...
            try:
                ...
                _LOGGER.error("hOn Invalid Data [" + text + "] after sending command [" + str(data) + "]")
                return False

            if json_data["payload"]["resultCode"] == "0":
                return True

            _LOGGER.error("hOn command has been rejected. Error message [" + text + "] sent data [" + str(data) + "]")
            #_LOGGER.warning(text)
